...
.vd-group__button--info { font-family: Georgia, 'Times New Roman', serif; font-style: italic; font-weight: 700; }
.vd-badge {
...
/* La leyenda ("Cómo leer el mapa") es la única excepción a "hover también abre el panel" (ver
   comentario sobre el toolbar más abajo) -- a pedido explícito, debe abrir solo con clic, para no
   taparle el mapa a quien solo pasó el mouse de camino a otro botón del toolbar. Los `:not()`
   son necesarios porque el cursor sigue "encima" del botón justo después de clickearlo -- sin
   ellos, esta regla (más específica que `.vd-group.vd-abierto .vd-panel`) taparía el panel que el
   propio clic acaba de abrir. */
.vd-group[data-grupo="leyenda"]:hover:not(.vd-abierto):not(:focus-within) .vd-group__button {
  background: #ffffff; border-color: #E6E0CD; color: #4B5048;
}
.vd-group[data-grupo="leyenda"]:hover:not(.vd-abierto):not(:focus-within) .vd-panel {
...
/* El slider dual (precio/dormitorios/etc.) posiciona `.vd-slider__fill` por JS entre sus dos
   thumbs (left Y width). El de Tamaño de los círculos es de un solo thumb -- el relleno siempre
   arranca en el borde izquierdo, así que `left` queda fijo acá y JS solo toca `width`. */
.vd-slider--simple .vd-slider__fill { left: 0; }
...
<div id="toolbar-filtros">
  ...
  <div class="vd-group" data-grupo="tamano">
    <button type="button" class="vd-group__button vd-group__button--icono" aria-label="Tamaño de los círculos" title="Tamaño de los círculos">⊙</button>
    <div class="vd-panel">
      <div class="vd-mega-field__head">
        <span class="vd-group__label">Tamaño de los círculos</span>
        <span class="vd-group__value" id="resumen-tamano">100%</span>
      </div>
      <div class="vd-slider vd-slider--simple">
        <div class="vd-slider__track"></div>
        <div class="vd-slider__fill" id="relleno-tamano"></div>
        <input type="range" id="rango-tamano" min="50" max="200" value="100" step="10">
      </div>
      <div class="vd-slider-values">
        <span>Pequeño</span>
        <span>Grande</span>
  ...
  <div class="vd-group vd-group--derecha" data-grupo="leyenda">
    <button type="button" class="vd-group__button vd-group__button--icono vd-group__button--info" aria-label="Cómo leer el mapa" title="Cómo leer el mapa">i</button>
...
// Multiplicador manual del slider "Tamaño de los círculos" (ver #rango-tamano más abajo) -- se
// combina con el factor de zoom en vez de reemplazarlo, para que el usuario pueda agrandar/achicar
// los puntos en cualquier nivel de zoom sin perder el ajuste automático que evita que se solapen al
// alejar el mapa.
var factorTamanoUsuario = 1;
...
function actualizarRadiosPorZoom() {{
    var factor = factorRadioPorZoom({nombre_js_mapa}.getZoom()) * factorTamanoUsuario;
...
var elTamano = document.getElementById('rango-tamano');
var rellenoTamano = document.getElementById('relleno-tamano');
var resumenTamano = document.getElementById('resumen-tamano');

function actualizarSliderTamano() {{
    var min = parseFloat(elTamano.min), max = parseFloat(elTamano.max), valor = parseFloat(elTamano.value);
    rellenoTamano.style.width = ((valor - min) / (max - min) * 100) + '%';
    resumenTamano.innerText = valor + '%';
    resumenTamano.classList.toggle('vd-group__value--activo', valor !== 100);
    factorTamanoUsuario = valor / 100;
    actualizarRadiosPorZoom();
}}
elTamano.addEventListener('input', actualizarSliderTamano);
...
function manejarClicMarcador(marcador, e) {{
    ...
    // El hover ya aclaró el color para dar sensación de "botón" -- al clickear, aunque el mouse
    // siga encima (o el marcador se reubique en el abanico de desenganche, ver desplegarGrupo,
    // donde el cursor nunca queda "sobre" la nueva posición y por lo tanto mouseout jamás dispara),
    // hay que forzar la vuelta al color original en vez de esperar un mouseout que podría no llegar.
    marcador.setStyle({{ color: marcador._datos.color, fillColor: marcador._datos.color }});
...
// Cache por color exacto (hoy solo 'red'/'green', ver color_por_residuo en Python) para no crear y
// destruir un <div> de medición en cada mouseover -- se resuelve el color vía getComputedStyle en
// vez de mezclar el string a mano porque el mapa no controla si el color llega como nombre CSS,
// hex o rgb().
var cacheColoresClaros = {{}};
function obtenerColorClaro(colorCss) {{
    if (cacheColoresClaros[colorCss]) {{ return cacheColoresClaros[colorCss]; }}
    var medidor = document.createElement('div');
    medidor.style.color = colorCss;
    document.body.appendChild(medidor);
    var rgb = getComputedStyle(medidor).color.match(/[0-9]+/g).map(Number);
    document.body.removeChild(medidor);
    var factorAclarado = 0.35;
    var claro = 'rgb(' + rgb.map(function(canal) {{ return Math.round(canal + (255 - canal) * factorAclarado); }}).join(',') + ')';
    cacheColoresClaros[colorCss] = claro;
    return claro;
}}
...
function enlazarClicMarcador(m) {{
    ...
    m.on('mouseover', function() {{
        m.setStyle({{ color: obtenerColorClaro(m._datos.color), fillColor: obtenerColorClaro(m._datos.color) }});
    }});
    m.off('mouseout');
    m.on('mouseout', function() {{
        m.setStyle({{ color: m._datos.color, fillColor: m._datos.color }});
    }});
}}
...
document.querySelectorAll('.vd-group__button').forEach(function(boton) {{
    boton.addEventListener('click', function() {{
        ...
        // Al cerrar, el botón sigue enfocado (el click no le quita el foco) y `:focus-within`
        // seguiría mostrando el panel aunque ya no tenga `vd-abierto` -- afecta sobre todo a la
        // leyenda, que depende solo del clic (ver `.vd-group[data-grupo="leyenda"]` más arriba),
        // ya que en el resto el hover disimula el problema.
        else {{ boton.blur(); }}
    }});
}});
document.addEventListener('click', function(e) {{
